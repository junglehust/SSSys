...
static void on_pre_conn(struct rdma_cm_id *id)
{
  ...
  //这里还是使用的一般内存的分配方式 要改成持久内存的
   posix_memalign((void **)&ctx->buffer, sysconf(_SC_PAGESIZE), BUFFER_SIZE); //按页大小对齐内存
...
static void on_completion(struct ibv_wc *wc)
{
  ...
  if (wc->opcode == IBV_WC_RECV_RDMA_WITH_IMM){
    ...
    if (size == 0) {
      ...
      //这里的write改成pmem_memcpy_persistent
      ...
      //类似的 把open改成pmem_map_file pmem_unmap这一套
